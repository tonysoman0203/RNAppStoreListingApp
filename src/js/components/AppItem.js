...
class AppItem extends Component<Props> {
	...
	renderAppRecommendItem = () => {
		...
		else imageHeight = parseInt(this.props.entry.image[2].attributes.height)
		...
		return (
			<View style={{ width: 100, height: 100, margin: 5 }}>
				...
				<Text style={styles.label}>
					{this.props.entry.category.attributes.label}
				</Text>
			</View>
		)
	}
	...
	renderNormalAppItem = () => {
		...
		return (
			...
			>
				...
				<Image
					borderRadius={
						parseInt(this.props.indexText) % 2 == 0 ? 50 : 20
					}
	...
	renderRating = () => {
		if (
			this.props.entry.averageUserRating &&
					this.props.entry.userRatingCount
		) {
			return (
